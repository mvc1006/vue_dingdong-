<template>
    <div class="box">
      <div class="nav">
        <el-form :inline="true" :model="ruleForm"  :rules="rules" ref="ruleForm"  style="margin-top: 22px; " class="demo-form-inline">
...
<script>
    export default {
      ...
      created(){
        ...
        //获取所有员工考勤列表
        axios.get('/api/Handler/User.ashx?type=Web_RiQDmingxi&Sch_dt='+this.time+'&Sch_bu=0&cPage=1&pSize=10')
          .then( (response)=> {
            console.log(response.data.data);
            this.tableData = response.data.data;
            this.loading = false;
      ...
      methods:{
        ...
        checkCondition(){
          this.loading = true;
          axios.get('/api/Handler/User.ashx?type=Web_RiQDmingxi&cPage=1&pSize=10&Sch_dt='+this.time+'&Sch_bu=0&Sch_type='+this.ruleForm.type+'&Sch_value='+this.value+'')
            .then( (response)=> {
              console.log(response.data.data);
              this.tableData = response.data.data;
              this.loading = false;
        ...
        handleCurrentChange(e){
          console.log(e);
          this.loading = true;
          axios.get('/api/Handler/User.ashx?type=Web_RiQDmingxi&Sch_dt='+this.time+'&Sch_bu=0&cPage='+e+'&pSize=10')
            .then( (response)=> {
              console.log(response.data.data);
              this.tableData = response.data.data;
              this.loading = false;
        ...
        downLoad(){
          ...
          let excle = '/api/Handler/Report.ashx?type=rimingxi&Sch_dt='+this.time+'&Sch_bu='+this.groupid+'&Sch_type='+this.ruleForm.type+'&Sch_value='+this.value;
